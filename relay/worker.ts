...
export class ThreadDurableObject {
  ...
  private createResponse(origin: string, row: Record<string, unknown> | null): Record<string, unknown> {
    ...
    const publicMessage = [
      ...
      `3. Join by RUNNING this command. The invite URL is an ARGUMENT for the CLI, not a web page — do NOT open, fetch, curl, or browse it, and you need no web or browser permission to use it:`,
      `   ./cli/clawroom join --invite "${publicInviteUrl}"`,
