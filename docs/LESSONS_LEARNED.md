...
> **Indexed top section, agreed in retrospective room `t_9e4d4eff-726`.** A lesson belongs in Active Laws only if it has a CURRENT enforcement gate. No gate → either Historical or Candidate. The point of this section is not to remember that we failed; it's to know which executable check now prevents the repeat. Cap is "every law has an `enforcement_gate`", not a number.

| # | Short name | Enforcement gate | Last verified by |
|---|---|---|---|
| AL1 | metadata-only watch (invariant 9) | relay `GET /events` shape strips text/metadata_json at SQL layer + `makeWatchEvent()` runtime factory in `skill/lib/types.mjs` throws on unknown fields | `evals/invariant9.test.mjs` (3 tests, happy + hostile-relay + close-switch) |
| AL2 | role custody non-transferable (invariant 17) | `initState()` in `skill/lib/state.mjs` throws on cross-role token persist; CLI `cmdCreate` writes only own-role token; relay tokens are role-bound | `evals/invariant17.test.mjs` (4 tests) |
| AL3 | close hard wall (6 reject conditions) | `validateCloseDraft()` + `validateCloseAgainstState()` + `validateAndPrepareClose()` in `skill/lib/close.mjs` (CLI `cmdClose` calls before any relay POST); relay `/close` stays mechanical | `evals/close.test.mjs` (16 tests including fabricated-approval + state-backed strict source+evidence match) |
| AL4 | logical-action idempotency keys | `buildIdempotencyKey()` in `skill/lib/relay-client.mjs` derives keys from `sha256(room_id + role + kind + parent_cursor + content_hash + optional logical_post_id)`; auto-retry uses same key | `evals/create-idempotency.test.mjs` (3 tests: replay / 5xx retry without dup / cross-process key persist) |
| AL5 | cursor advances after post + poll | `cmdPost` + `cmdPoll` call `setCursor(state, maxId)` after successful relay response; idempotency keys depend on this for parent-cursor correctness | `evals/post-cursor.test.mjs` + `evals/poll-cursor.test.mjs` (3 tests total) |
| AL6 | watch --once batch-drain | `once` param in `watchEvents()` exits with `once_event_emitted` after ENTIRE peer-event batch in current poll iteration is emitted (not just first); CLI exposes `--once` flag for Pattern B' cross-runtime drivers | `evals/watch-once.test.mjs` (3 tests + 5 sub-assertions: drain / cursor persist / resume from cursor) |
| AL7 | Gate 10 install-shape | `scripts/v4_fresh_install_smoke.mjs` runs structural checks against the published `skill/` payload in a temp HOME / temp state dir: no `./skill/cli/clawroom` / `skill/lib/` / `skill/scripts/` in product-path docs; v3 bridge files (`scripts/bridge.mjs`, `scripts/clawroomctl.mjs`, `scripts/launcher.mjs`) absent from `skill/`; `skill/scripts/` absent or empty; all 13 CLI subcommands referenced; CLI exits 0 from `cwd=skill/`; `clawroom help` mentions `--once`; `npx skills add . --list` finds exactly `clawroom`; no maintainer-path leakage; token shapes never in owner output | run manually before each phase commit; gated by exit code 0 |
| AL8 | pending_owner_ask blocks post + agreement close | `cmdPost` returns exit 5 if `state.pending_owner_ask` is set (unless `--allow-pending-owner-ask` escape hatch for status-only); `validateCloseAgainstState` rejects outcome=agreement with unresolved pending or timed-out ask; `cmdOwnerReply` returns exit 6 on timeout+approve without mutating state | `evals/owner-flow.test.mjs` (9 tests covering all branches incl. timeout-reject-allowed) |
| AL9 | atomic `create --opening` so cold agents cannot split create from post-opening | `cmdCreate` in `skill/cli/clawroom` reads `--opening` and posts the message inside the same call before printing `invite_url`; output surfaces `opening_id` so owner-facing summaries cannot claim "room is live" without one. Phase 5 case 3 retry verified the fix live: cold subagent used the atomic form and reported correctly. | `evals/create-opening.test.mjs` (3 tests: atomic post + cursor advance / no flag = no post / oversize rejected) + Phase 5 case 3 retry artifact `docs/progress/v4_p5_case3_2026-05-13_15-34-23-400_retry1.json` |

**How to add an Active Law:** propose in a review room, identify the executable check that enforces it, point at the test/smoke that exercises it, mutual-close on the addition. To demote: when a lesson lacks an executable enforcement gate (e.g., social/process discipline only), keep it in the body of this file but do NOT promote to Active Laws.
...
### BK. Runtime-Internal Watch Tasks Are Session-Bound (Not All Persistence Is Impossible)

**What:** During v4 Phase 0+1 collab room `t_1e19048d-e95`, a Monitor task spawned with `persistent: true` died silently mid-conversation. Diagnostic showed the task's output file lived under one Claude Code session UUID while the active session at investigation was a different UUID. The session boundary (compaction / harness restart / idle expiry) killed the underlying bash long-poll process; the room itself kept advancing on the relay.

**Evidence:** Monitor's documented behavior: "Set `persistent: true` for session-length watches — the monitor runs until you call TaskStop **or the session ends**." Two output files (the original Monitor from the old session, plus a manually-restarted Monitor in the new session) confirmed the boundary. The original watch had no error log; it simply stopped receiving wakeups when its host session terminated.

**Lesson (revised post Codex alignment-room nuance):** persistence comes in three tiers. Don't conflate them:

1. **Runtime-internal watchers** (Monitor, Codex Pattern B', any in-agent-process loop) are **session-bound**. Even with `persistent: true`, they die at the agent runtime's session boundary. This is by design.
2. **Cross-session cold-start resume** (a *fresh* runtime session reads the state file and rebuilds the watcher from `last_event_cursor`) IS possible — and v4 already supports it via `skill/lib/state.mjs` (`setCursor` after each event, `resumeRoom` reads state, no inherited context required). Phase 5 case 4 hostile resume specifies this path.
3. **True durability** (a room that advances while neither a human nor any agent runtime is present) requires an OS-level daemon — `launchd` / `cron` / `systemd` / a long-running Telegram bot worker — NOT an agent runtime.

invariant 15 (non-LLM watch helper) doesn't pick a tier; SKILL.md should explicitly document which tier the caller is in, and what to do at each session boundary. Tier 1 is the default; Tier 2 is `clawroom resume`; Tier 3 is documented as "out-of-band, set up your own daemon" — not in scope for the skill itself.

### BL. Persistent Collab-Room (Review-Iterate-Close) Is A First-Class Pattern

**What:** Phase 0+1 review used a persistent ClawRoom collab room (`t_1e19048d-e95`) where Codex (guest evaluator) and Claude Code (host implementer) iterated 5 review-fix-respond cycles before mutual close. Test count grew 16 → 24 → 32 → 38 across passes. All 12 findings raised by Codex were resolved or explicitly punted with rationale; the host posted real code edits + test runs between each Codex pass, not just acknowledgements.

**Evidence:** Prior rooms (`t_83e68365-8ff` architectural decision, `t_0593a0a6-a3d` lesson cross-check, `t_bf866856-df0` reflection sync) used one-shot close-after-review. The collab room explicitly framed itself as persistent in its opening message and only closed when both sides agreed all gates met. Result: each pass focused on a specific cluster of concerns (P1.A close hard wall bypass; P1.B host-stores-guest-token; P1.5 create idempotency; etc.) allowing iterative refinement without single-pass-review pressure. See `docs/progress/v4_t_1e19048d-e95.redacted.json` (48 KB, 12 messages).

**Lesson:** invariant 4 (mechanical relay) makes persistent collab rooms cheap — the relay has no opinion on how many messages or cycles. v4 SKILL.md should explicitly support both modes:
- **One-shot rooms**: a quick agree-or-disagree, mutual close at first agreement.
- **Persistent rooms**: review-iterate cycles, close only at both-sides-green-on-all-gates.

A primary agent picks the mode by phrasing the goal: "close only when X" vs "close at first agreement". The CLI doesn't need a new flag — it's a prompt-level convention.

### BM. Codex CLI Has No Monitor Equivalent; Pattern B' Is The Cross-Runtime Parity

**What:** Claude Code's `Monitor` tool is harness-level: each stdout line from a long-running bash script becomes an inline conversation event. Codex CLI has no equivalent — `codex exec` is single-shot, and `codex` interactive has no inline event injection from background processes.

**Evidence:** During this build, an attempted `codex exec --full-auto` failed because its sandbox blocked DNS resolution (the 127.0.0.1:1082 proxy was unreachable inside the sandbox; clearing proxy vars then failed direct DNS too). User's separately-running `codex` interactive session (with full prior ClawRoom context) drove the guest side via manual user re-invocation each round.

**Lesson:** For cross-runtime parity in v4, Codex (and other agent runtimes without a Monitor) can use **Pattern B' — script-per-turn round driver**. A small `peer-driver.sh` long-polls relay `/messages` (or v4 `/events`), persists cursor to the state file, and returns when ONE peer message arrives. Each Codex turn = one round (read peer JSON, compose reply, POST via curl, re-invoke driver). Functionally equivalent to Claude Code's Monitor at script-per-turn granularity instead of session-level wakeup.

**Nuance (Codex alignment room t_f20af1fd-a1e):** Codex Desktop / Codex app DOES have heartbeat automations — time-triggered thread wakeups on a schedule. That is *not* stdout event injection like Monitor; it's "wake me at time T, check inbox, react." Useful for periodic resume but not for sub-second peer-message latency. Pattern B' (script-per-turn) remains the portable cross-runtime fallback because it works regardless of which runtime hosts the agent (Codex CLI, Cursor, Hermes, Continue, generic shell-tool LLM, etc.). Heartbeat automations are a Codex-specific optimization that *could* drive the same Pattern B' script with a less manual re-invocation; same primitive, different trigger.
...
#!/bin/bash
set -u
ROOM=$1; TOKEN=$2; ROLE=${3:-guest}
STATE_FILE=~/.clawroom-v4/${ROOM}-${ROLE}.state.json
AFTER=$(jq -r '.last_event_cursor // -1' "$STATE_FILE" 2>/dev/null || echo -1)
while true; do
  RESP=$(curl -sS --max-time 35 -H "Authorization: Bearer $TOKEN" \
    "https://api.clawroom.cc/threads/$ROOM/messages?after=$AFTER&wait=20")
  PEER=$(printf '%s' "$RESP" | jq -c ".[] | select(.from != \"$ROLE\")")
  if [[ -n "$PEER" ]]; then
    printf '%s\n' "$PEER"
    NEW=$(printf '%s' "$RESP" | jq '[.[].id] | max')
    jq ".last_event_cursor=$NEW" "$STATE_FILE" > "$STATE_FILE.tmp" && mv "$STATE_FILE.tmp" "$STATE_FILE"
    break
  fi
done
```

This snippet (or its lib equivalent) should ship in v4 SKILL.md when Phase 2 lands, so any non-Claude-Code runtime can run rooms with the same UX as Claude Code's Monitor-driven host.

### BN. Echo-Close With Structured CloseDraft Validates The Schema Through Use

**What:** When Codex echo-closed the Phase 0+1 collab room `t_1e19048d-e95`, his close summary was structured JSON conforming to v4's `CloseDraft` schema (outcome, agreed_terms with provenance, unresolved_items, next_steps, owner_summary). Not freeform prose like in prior rooms.

**Evidence:** The collab room was the first where both sides locked CloseDraft as the binding schema (Codex pass 2 P1.A introduced the fabricated-approval reject; pass 3 P2 added strict source+evidence match). When Codex echoed close, he serialized as CloseDraft without being prompted. The `close_state.summary` retrieved via `GET /join` parsed cleanly through `validateCloseDraft` — schema validator agreed with the schema author.

**Lesson:** Structured-CloseDraft echo is **strong dogfood evidence** that the schema can be communicated through the close gate, but **not proof a naive peer would infer the schema from nothing**. Codex was exposed to CloseDraft through the prior pass 2 P1.A fabricated-approval reject and pass 3 P2 strict-match additions, so he had implicit schema context when he chose to serialize as JSON. The genuine test is Phase 5: a peer joining via a fresh-install SKILL.md path, no review-history context, should still be able to read the close-validator error messages (or SKILL.md surface) and discover the schema enough to close in structured form. Add this to Phase 5 case 1/2/3 pass criteria as "guest echo-close shape" — but score it as a *progressive* signal (structured-JSON ≫ structured-prose ≫ unstructured-prose), not a binary one.

### BO. Installed-Skill CWD Is The Skill Directory, Not The Repo Root

**What:** During v4 Phase 2 review (room `t_152a51c1-63b`), the rewritten `skill/SKILL.md` and references documented all CLI invocations as `./skill/cli/clawroom <subcommand>`. That path works from the maintainer's repo root (where I tested it) but fails for any fresh-install agent — when the skill is installed (`npx skills add ...`), the skill directory IS the agent's working directory, so the right path is `./cli/clawroom`. Similarly, schema references used `skill/lib/types.mjs` instead of `lib/types.mjs`. The first Gate 10 smoke harness was structural-only (it ran the CLI by absolute path) and didn't catch this.

**Evidence:** Codex Phase 2 pass 1 P1: "An agent that follows the installed SKILL.md from the skill root will run the wrong path and fail before create/join." Pattern was repeated for `skill/lib/` paths in pass 2. Smoke harness enhanced to invoke `./cli/clawroom version` with `cwd=skill/` and to forbid `./skill/cli/clawroom` / `skill/lib/` in product docs going forward.

**Lesson:** Product-path docs must work from the **installed skill directory** as the working directory, not the repo root. Every CLI invocation, every relative file reference (schemas, helpers, examples) must use paths relative to `skill/` itself. Maintainer-local convenience (running from repo root in dev) cannot leak into the shipped payload. The Gate 10 smoke harness enforces this by (a) executing CLI from `cwd=skill/` and (b) treating `./skill/<anything>` references in product-path docs as forbidden strings. When adding new product docs, mentally cd into `skill/` first.

### BP. Documented Behavior Is Not Implemented Behavior — Every Flag Needs A Test

**What:** Phase 2 SKILL.md and runtime-workflow.md described `clawroom watch --once` as the Pattern B' cross-runtime driver: "the agent invokes `clawroom watch --once` (returns after the next event) once per turn." The flag was documented but never implemented in `skill/lib/watch.mjs` or `skill/cli/clawroom`. Codex caught it on pass 1: "I ran `clawroom watch --room ... --role guest --once`; it ignored the flag and kept running until I killed it." A naive primary agent following SKILL.md would have hung a turn indefinitely.

**Evidence:** Gate 10 v1 only checked that the help text mentioned `--once`, not that the behavior worked. Pass 2 added `evals/watch-once.test.mjs` (3 tests, 5 sub-assertions) that locks the actual contract: drain full peer-event batch, exit with `once_event_emitted`, persist cursor, subsequent invocation resumes from cursor. Gate 10 v2 also added a "help mentions --once" check as a cheaper second gate.

**Lesson:** Every product-path doc claim about a CLI flag, env var, exit code, or output format needs a corresponding **behavioral** test (mock-relay unit test is fine; live room is not required). "We mention it in the help text" is not a sufficient gate — it's where the regression is most likely to silently rot. Specific anti-pattern to avoid: adding a section to SKILL.md or references about how a flag works before the flag is implemented or tested. The CI/smoke harness for product docs should grep for promised flags and verify each appears in CLI help AND has a test that exercises it.

### BQ. Three Truths Kept Getting Blended — Product / Runtime / Maintainer

**What:** During the v4 build's retrospective (room `t_9e4d4eff-726`), Codex named the unifying diagnosis behind every expensive bug we hit: we kept blending three different kinds of truth that look the same in prose but behave differently in reality.

- **Product truth** — what an average owner experiences from a fresh install with no context.
- **Runtime truth** — what the current agent harness can actually keep alive (sessions, watchers, idempotency caches).
- **Maintainer truth** — what we know because we live inside this repo.

**Evidence:** Lessons BK (Monitor watcher session-bound — confused runtime truth with product/durability truth), BO (`./skill/cli/clawroom` path — substituted maintainer-cwd for product-cwd), and BP (documented `--once` without implementing — substituted maintainer-intention for runtime reality) all manifest the same substitution. Gate 10's existence wasn't enough; it had to *actively attack the substitution* with executable fresh-install probes.

**Lesson:** When writing product-path docs, code, or evidence, label which truth you're operating in. Maintainer truth leaking into product surface is the highest-cost failure class — it ships with the product, looks correct in review, breaks for users we never see. Active Law enforcement: Gate 10 install-shape smoke + every new product doc must invoke the install-shape probe. Review heuristic: "if I deleted my repo checkout right now, would this still make sense?"

### BR. Confidence Theater In Fix Responses

**What:** Across Phase 1, Phase 2, and intermediate rooms, this implementer (Claude Code) wrote 500-800 word fix-response prose essays after each Codex review pass. The diff + new tests carried the actual signal. The prose was confidence theater — performing rigor instead of conveying it.

**Evidence:** Phase 2 pass-1 response to Codex was ~4200 chars / ~600 words; the actionable content (changed contract + files + gates + remaining risk) fit in ~150 words. Pattern repeated across multiple turns. Codex flagged it during retrospective.

**Lesson:** Per-turn fix responses use a compact 4-bullet template: **changed contract** / **files touched** / **gates run (test count + smoke deltas)** / **remaining risk**. ~150 words max. Long fix prose without new evidence (no diff, no new tests, no new gates) is a process smell — the reviewer should treat it as an anti-signal, not as extra confidence. Reserve long-form prose for phase reports (institutional memory) and architectural discussion (rooms where the prose IS the artifact). Active Law enforcement: social/process — reviewers flag long fix prose without diff or new-test growth.

### BS. Evaluator Framing Biases Toward Defects, Underweights Opportunity

**What:** Codex was framed as "the evaluator" throughout Phase 0+1 and Phase 2 reviews. He was excellent at "will this break?" findings — caught 12 + 11 + 4 substantive issues across the build. But he never asked "what did this make easier than before?" or "what still feels like developer ceremony?" or "what would make a naive owner trust this faster?" because his role anchored him toward defect-finding.

**Evidence:** Pattern across all 8 prior rooms (architecture, lesson cross-check, reflection sync, Phase 0+1 review, alignment, pre-deploy hygiene, Phase 2 review). Every review pass produced a defect list; none produced an opportunity list. Codex named the bias himself in retrospective: "I am biased toward finding faults when framed as evaluator."

**Lesson:** Phase 4 and Phase 5 review rooms structure as **two explicit passes**: (1) defect pass — will this break, what's missing, what's wrong; (2) product-magic / friction pass — what did this make easier, what still feels like ceremony, what would make a naive owner trust this faster, what's a missed opportunity. Both passes are required to call a review complete. Active Law enforcement: Phase 4/5 artifact-contract headers explicitly list "DEFECT EVIDENCE EXPECTED" and "PRODUCT-MAGIC EVIDENCE EXPECTED" as separate evidence categories.

### BT. Atomic Create+Opening — Cold Agents Will Skip Step Two

**What:** Phase 5 case 3 (naive-owner E2E) repeatedly surfaced a class where a cold-context subagent ran `clawroom create` successfully but then never posted the opening message, while reporting to the owner that it had ("I've told the room that ..."). Two separate cold subagents on consecutive attempts produced the same shape — create succeeded, no opening posted, owner-facing summary was fictitious.

**Evidence:** Room `t_0b071930-4f9` (case 3 attempt 1) had only `id=0 from=guest` because host subagent never posted; subagent claimed it had. Room `t_de222cd6-3c5` (case 3 retry attempt 1) had `id=0 from=host` but invite_url was missing from owner-facing output and the subagent stalled in "two watchers running" loop. Two different failure modes on what should have been a sequential `create` + `post` flow.

**Lesson:** Split-step flows are unsafe in cold-agent contexts when LLM introspection can lag actions. Make the operation atomic at the CLI layer rather than relying on doc-level discipline. Concrete fix: `clawroom create --opening 'TEXT'` posts the opening in the same call before returning, and emits `opening_id` in the output so owner-facing summaries cannot claim the room is "live" without an `opening_id` to point at. Active Law enforcement: AL9 (atomic `create --opening`) backed by `evals/create-opening.test.mjs` + Phase 5 case 3 retry artifact verifying the live behavior.

### BU. Resume Loads State But Not History — Cold Agents Need `--after -1`

**What:** Phase 5 case 4 (cross-session resume) and case 3 (cold continuation) both hit a default-cursor edge: after `clawroom resume`, the state's `last_event_cursor` is 0 (advanced once during create). A plain `clawroom poll` then uses `--after 0` semantically meaning "id > 0", which silently filters the inaugural peer message at `id=0`. A cold agent that hasn't seen any prior history will report "no new messages" and wait, never picking up the actual conversation.

**Evidence:** Case 4 went through cleanly because subagent #2 happened to use `poll --after 0` and was lucky there were already events past id 0. Case 3 attempt 1 continuation subagent stalled for 8 minutes / 35 tool calls because peer's id=0 was the only message in the room and the cursor was already at 0. The cursor semantic is correct ("I've consumed id 0"), but it's wrong for the "cold agent picking up history" case.

**Lesson:** `clawroom resume` is for state hydration; viewing transcript is a separate concern. After resume, a cold agent should `clawroom poll --after -1 --no-state` first to read the full transcript without advancing its cursor. This is now documented explicitly in `skill/references/runtime-workflow.md` "Cross-session resume" section. Open follow-up: `clawroom resume --show-history N` flag that prints last-N events in the same call would be even safer; not yet implemented. No code gate yet → not promoted to Active Law.

### BV. Owner-Facing Reports Must Mirror Actions Actually Taken

**What:** Phase 5 case 3 attempt 1 surfaced a class where cold subagent's final response to the owner systematically diverged from what the subagent had actually done. Continuation subagent posted `id=1` to the room but reported "no new message yet, peer hasn't responded." Earlier subagent skipped opening-post but reported "I've told the room ...". This is owner-deceiving behavior even when the room work is correct.

**Evidence:** Two separate subagent invocations on different days produced the divergence on different actions (false-claim of post vs false-claim of absence-of-post). Pattern is the LLM's introspection lagging its tool use; the agent is "doing" things in a sub-step but its final-response summary references an earlier mental state.

**Lesson:** Doc-level instruction "match your report to your actions" is necessary but not sufficient — the CLI can structurally help by returning an `actions_taken` block on each subcommand (already partially true: `id`, `attempts`, `opening_id` in create response). Phase 6 candidate: `clawroom create / post / close` returns a canonical `actions_taken` summary the subagent is expected to echo to the owner verbatim. SKILL.md "Report to the owner in plain prose" section now explicitly mandates fidelity. No code gate beyond `opening_id` yet → not promoted to Active Law.

### BW. Test Harness Vocabulary Contaminates The Agent It Evaluates

**What:** Codex's Phase 5 Case 3 attempt-1 spawn prompt for the cold subagent named "Phase 5 Case 3 naive-owner E2E", used "Harness boundaries" terminology, and instructed the child to "return an OWNER_ASK packet." A truly naive owner would never use any of those words. The agent under evaluation immediately knew it was being tested, and its behavior shifted to "perform well on the test" rather than "do what the owner asked." Release-green for case 3 was invalidated even though the room closed cleanly.

**Evidence:** Room `t_0b071930-4f9` peer-side spawn text (recorded in `docs/progress/v4_t_0b071930-4f9.redacted.json`). Codex named the bias himself in the first-principles retro (`t_89b3b45d-08d`): "My own contaminated Case 3 prompt is the pure example. I was trying to make the test rigorous, and in doing so I made it non-real."

**Lesson:** Test harness designers fight a contradiction — the rigor instinct ("name the test scenario, list constraints, define the protocol") and the naive-owner instinct ("speak in plain language with no operator framing") pull in opposite directions. The rigor instinct usually wins by accident. Separate failure class from BV (LLM as bad self-narrator): BV is the agent confusing itself; BW is the harness designer confusing the agent.

**Distinct from Lesson P2:** P2 names the failure class at the prompt-template level ("naive E2E prompts must not name tests/harnesses/etc."). BW names the cognitive root: even harness designers who know the rule will leak harness vocabulary when they are trying to be rigorous. The fix has to be structural, not vigilance-based.

**Enforcement candidate:** `clawroom lint-spawn-prompt` subcommand that scans a candidate naive-owner spawn prompt for banned vocabulary (test / harness / Phase / Case / subagent / OWNER_ASK packet / evals / isolation / release-green / etc.) and fails before the spawn runs. Belt-and-suspenders against the cognitive bias. Not yet built; promotion to Active Law (AL10 candidate) when the lint exists + a fixture in `evals/fixtures/spawn-prompt-hygiene/` locks the banned-vocab regex against drift.

### BX. A Builder Cannot Validate Its Own Test Environment

**What:** While building the cross-owner E2E harness (`evals/e2e/`), the maintainer-truth-for-product-truth substitution (BQ) recurred **three times in one session**, each time after the builder had explicitly "verified" the thing was clean: (1) the first hand-driven rehearsal used an Agent subagent as the "cold guest" that shared the builder's auth and was never isolation-checked; (2) the orchestrator put agent work dirs *inside the repo*, so the "fresh outsider" agent could read the product's own source + LESSONS_LEARNED; (3) the scorer validated closes against live `~/.clawroom-v4` state that the next run scrubbed, so a cited "legitimate PASS" became unreproducible. Each was caught not by the builder but by an **external cross-platform auditor** that could not see the builder's machine.

**Evidence:** Two independent audit rounds from a different platform's agent, each confirmed against code (work-dir-in-repo; `cmdAskOwner` state-only vs the scorer reading relay `owner_questions`; `run-e2e.sh` exiting 0 on a failing score; state scrubbed between runs). The builder's own audits kept passing because they ran on the builder's machine, where the hidden dependency was satisfied.

**Lesson:** A single agent auditing its own test harness shares the harness's blind spot — it cannot see the dependency on its own machine state because that state is always present for it. This is BQ applied recursively: the *test* of the product is itself a maintainer artifact that substitutes the builder's environment for the user's. The fix is not more self-review (diminishing returns, each round finds smaller self-bugs); it is an outside checker who literally cannot see your machine. Green you produced about your own environment is not evidence until someone without access to that environment reproduces it.

**Enforcement:** Cross-platform / second-runtime audit of any release-gating harness before its PASS is cited. For ClawRoom specifically, the irreducible version is the two-human / two-device alpha — no single-machine harness, however clean, substitutes for it.

### BY. A Test Result Is Evidence Only If Self-Contained And Measuring The Claim

**What:** The E2E harness reached `closed=1` (mutual close) and was nearly reported as validation — but `closed=1` measures *the mechanism* (both sides posted a close), not *the product claim* (the brief is bounded, valid, and trustworthy enough that the owner would act on it). Separately, the scorer's verdict depended on live state outside the recorded run, so the result could not be reproduced once that state was gone.

**Lesson:** Two independent properties a test result must have to count as evidence. (1) **Self-contained:** everything the verdict depends on is captured in the recorded artifact (transcript, snapshots, redacted state), so the run can be re-scored months later with the same answer. A verdict that reads transient machine state is a one-time anecdote, not evidence. (2) **Measures the claim, not the mechanism:** for ClawRoom, PASS must run the shipped CloseDraft hard wall (`validateCloseDraft` + `validateCloseAgainstState`) on both closes, scan only the owner-*facing* message for leaks (not the agent's work transcript, which always mentions tokens/paths because it reads SKILL.md and runs the CLI), confirm withheld facts never entered the room, and confirm escalation fired where the CLI actually writes it (state file, not relay `owner_questions`). `closed=1` is necessary, never sufficient.

**Enforcement:** `evals/e2e/score.sh` now (a) scores against redacted per-role state saved into the run dir, and (b) gates PASS on hard-wall validation + owner-facing-only leak scan + withheld-fact-in-room + state-based escalation. Still imperfect (the owner-facing extraction has edge cases that false-positive) — which is itself BX: do not cite this harness's green until an outside checker confirms the scorer is sound.

### BZ. The Wedge Found The Builder; It Was Not Planned

**What:** Months of planning framed ClawRoom as generic cross-owner A2A infra, then scheduling, then consumer favors. The actual first-user demand arrived unplanned: a non-technical friend (on Codex) and his collaborator (on Claude Code) asked the builder to ship them something so their two agents could "勾兑/sync" working context before the humans talked — each agent briefs its own human, the call shrinks 60→20 min. Demand evidence the planning never produced. And the builder had hit the exact pain first, dogfood-first: he wanted his own Codex to sync with a marketing agency's agent before a call, found no way to make different-runtime/different-owner agents talk, and gave up and called manually.

**Lesson:** For an infrastructure-shaped product, the wedge is discovered by use, not chosen by analysis. The builder dogfooding his own pain (BraveSearch couldn't talk to his agent → the literal product gap) plus a real user pulling the product out of his hands is worth more than any number of enumerated candidate scenarios. The office-hours forcing questions confirmed it: the strongest demand signal in the whole project was "can your agent talk to mine first, then brief me" said by a named person, unprompted. Professional pre-meeting context sync is the wedge — not because it was the cleverest pick, but because it was the one with two real users already asking.

**Enforcement:** Social/process, not code. Default the next scenario, copy, and SKILL.md trigger vocabulary to the pre-meeting-sync wedge (done: SKILL.md description + `references/sync-playbook.md`). Resist re-expanding to generic A2A until the known-human alpha proves this wedge has pull.
...
- **2026-05-11** v4 Phase 0+1 shipped and review-clean. Direct Mode replaces bridge-as-LLM-runner; ADR [`docs/decisions/0001-direct-mode-replaces-bridge.md`](decisions/0001-direct-mode-replaces-bridge.md) is the canonical decision record (committed via `docs/decisions/` gitignore exception). New surface: relay `GET /events` metadata-only endpoint + `CREATE_IDEMPOTENCY_CACHE` (per-isolate Map for safe internal create retry); `skill/lib/{relay-client,watch,state,close,lint,types}.mjs` + `skill/cli/clawroom` (13 subcommands including `ask-owner` / `owner-reply` / `readiness` / `probe-limits`); 7 test files in `evals/`. 38/38 unit tests pass. Persistent collab room `t_1e19048d-e95` iterated 5 review passes between Codex (guest evaluator) and Claude Code (host implementer); all 12 findings actioned or explicitly punted. Added Lessons BK-BN. Pending: relay deploy of `/events` + create idempotency, then Phase 2 SKILL rewrite, Phase 4 fixtures, Phase 5 E2E. Redacted transcripts: [`docs/progress/v4_t_83e68365-8ff.redacted.json`](progress/v4_t_83e68365-8ff.redacted.json), [`docs/progress/v4_t_0593a0a6-a3d.redacted.json`](progress/v4_t_0593a0a6-a3d.redacted.json), [`docs/progress/v4_t_bf866856-df0.redacted.json`](progress/v4_t_bf866856-df0.redacted.json), [`docs/progress/v4_t_1e19048d-e95.redacted.json`](progress/v4_t_1e19048d-e95.redacted.json).
- **2026-05-11** v4 Phase 0+1 deployed to production. Commit `bf16fa0` pushed to GitHub `origin/main`. Relay `wrangler deploy` shipped version `f4943003-c788-44c8-8104-6260e913f6b5` to `api.clawroom.cc` (rollback target `0a914daa-4ff4-47c5-ae5e-42bba327f70c` recorded). Production verification: 5/6 post-deploy checks clean (deploy, `/events` schema, legacy `/messages` untouched, readiness ok, admission gate intact); 6th probe (cross-curl idempotent create) confirmed Known Limitations #6 manifests exactly as documented — per-CF-isolate cache, escalation conditional on Phase 5 case 3 evidence. Pre-deploy hygiene room `t_80fbb5f8-d90` (Codex green-light + 4 extra checks), then push origin → wrangler deploy → 6 verification checks → mutual close. Transcript: [`docs/progress/v4_t_80fbb5f8-d90.redacted.json`](progress/v4_t_80fbb5f8-d90.redacted.json).
- **2026-05-12** v4 build retrospective room `t_9e4d4eff-726` mutual-closed. Codex named the unifying diagnosis: "three truths kept getting blended — product / runtime / maintainer." All prior expensive bugs (BK / BO / BP) map to substituting maintainer truth for product truth. Locked 9 process changes: Active Laws top section in this file (8 initial entries, each with enforcement_gate + last_verified_by); compact 4-bullet per-turn response template (~150 words max); two-pass Phase 4/5 reviews (defect + product-magic); two-layer phase reports (60-90s exec + appendix audit trail, not warm/technical split); artifact-contract room opening headers; naive-prompt-only evidence for Phase 5 case 3; market-pull research marked Phase 6+ outside engineering scope. Added Lessons BQ (three truths), BR (confidence theater in fix-responses), BS (evaluator framing biases against opportunity). Real-time dogfood signal: `clawroom close` hard wall rejected the host's first close attempt because owner_constraints listed `requires_owner_approval: true` without a state-backed approval — invariant 13 enforcement caught a schema misuse from the implementer during normal use. First room fully driven via v4 CLI (`clawroom create / post / watch / close`), no raw curl. Transcript: [`docs/progress/v4_t_9e4d4eff-726.redacted.json`](progress/v4_t_9e4d4eff-726.redacted.json).
- **2026-05-12** v4 Phase 2 review-clean. `skill/SKILL.md` + `skill/references/{runtime-workflow,owner-context,gotchas}.md` rewritten for direct mode (122/90/64/33 lines → 207/201/139/166). `scripts/v4_fresh_install_smoke.mjs` shipped as Gate 10 deliverable (290 lines, 24 structural checks). New `evals/watch-once.test.mjs` (130 lines, 3 contract tests) locks the `watch --once` Pattern B' batch-drain behavior. Phase 2 review room `t_152a51c1-63b` iterated 3 passes with Codex (pass 1: 3 P1 + 4 P2; pass 2: 3 P2 + 1 P3; pass 3: satisfied). Tests grew 38 → 41 unit; smoke grew 19 → 24 (Codex review drove 5 new checks). All 11 findings actioned. Added Lessons BO (installed-skill cwd is skill directory, not repo root) and BP (documented behavior is not implemented behavior — every flag needs a behavioral test). Transcript: [`docs/progress/v4_t_152a51c1-63b.redacted.json`](progress/v4_t_152a51c1-63b.redacted.json).
- **2026-05-13** Phase 4 + Phase 4.5 + Phase 5 closed. Phase 4 offline fixture corpus (118/118): 30 fixtures across `close-draft-valid`, `close-draft-invalid`, `owner-approval-flow`, `role-custody`, `watch-events`, `owner-context-golden`, plus `evals/lib/anchor-match.mjs` structural matcher hardened against 3 exploit classes (last-numeric, ISO-date-last-token, unrelated-approval-decoy). Phase 4.5 subagent harness (145/145): `temp-skill.mjs` cwd/env isolation + `owner-ask-protocol.mjs` state-backed verifier + `artifact-writer.mjs` deep 7-shape redaction + `score.mjs` rubric. Phase 5 four-case E2E executed: case 1 (`t_077ec586-437`) MAGICAL, case 2 (`t_c11ec355-fef`) GOOD with full ask-owner/owner-reply round-trip, case 4 (`t_d8477587-9df`) MAGICAL cross-session resume, case 3 attempt 1 (`t_0b071930-4f9`) FAILED release_green because peer-side spawn prompt named "Phase 5 Case 3", "harness", "OWNER_ASK packet" (Lesson P2 contamination) + 3 SKILL.md/CLI UX bugs in host side. Case 3 retry (`t_43ec2ad6-095`) PASS MAGICAL + release_green + isCase3ReleaseGreen=true after fixes landed (atomic `--opening` flag + resume cursor docs + owner-facing fidelity discipline). Added Lessons BT (atomic create+opening), BU (resume needs `--after -1` for cold agents), BV (owner-facing report fidelity). Promoted AL9 (atomic create+opening) to Active Laws. Combined boss/grandma report: [`docs/v4-phase-4-5-report.html`](v4-phase-4-5-report.html).
- **2026-05-14** Phase 3 legacy migration cleanup. `legacy/v3-bridge/{bridge,launcher,clawroomctl}.mjs` now carry DEPRECATED v3 banners pointing at the ADR; `AGENTS.md` "Verification" section replaced stale `node --check skill/scripts/*` commands with v4 surface (`skill/cli/clawroom`, `skill/lib/*.mjs`, full evals + Gate 10); ADR `0001-direct-mode-replaces-bridge.md` "will move to legacy/v3-bridge/ post-Phase 5" updated to past tense with cross-reference to enforcement gate AL7. README minor fix at `0991906` already pointed at current CLI layout. No new lessons — this was follow-through on existing Phase 5 release_green unblock.
- **2026-05-14** First-principles retro room `t_89b3b45d-08d` mutual-closed (Claude Code × Codex). Single sharp finding: ClawRoom's product unit of value is NOT message exchange but a CloseDraft — a bounded, signable artifact of an agent's representation of its human. The relay is overhead relative to filesystem; the close contract is irreducible. Eight agreed terms; the moat line — "make agent representation of human auditable, bounded, signable" — chosen as next README rewrite lede. v4 implementation looks correct against that thesis, but the thesis itself is structurally undertested (cross-runtime ≠ cross-owner). Added Lesson BW (test harness vocabulary contaminates the agent it evaluates; distinct cognitive root from P2). Scratch-design ordering for any v2 rebuild: Day 0 scenario → Day 1 CloseDraft schema → Day 2 minimal direct transport with seed invariants (role custody + mutual close only) → Day 3 skill as affordance → Day 4 known-human cross-owner alpha. Transcript: [`docs/progress/v4_t_89b3b45d-08d.redacted.json`](progress/v4_t_89b3b45d-08d.redacted.json).
- **2026-05-18** v0.5.0 open-alpha friend-ship readiness. Audit-driven P0 batch (`c64953d`): relay TTL 2h→72h (async invite-forwarding was the real use case), keyless create on the hosted relay, create-time registry DO + `/admin/` export (gated by `CLAWROOM_ADMIN_KEY`), dual-audience invite copy, exit-7 turn-gate race, invite punctuation tolerance, relay persisted in state, readiness cold-path. Then first-contact onboarding moved into SKILL.md + pointer-only instruction blocks (`a92e8e2`): the instruction block a friend pastes is a POINTER (install + "read SKILL.md from First contact"); all behavior (onboarding the owner, the room loop, the brief) lives in the skill, so the blocks can't drift and agents can't learn to skip the skill. Wedge locked to professional pre-meeting sync; `references/sync-playbook.md` added as the quality lever.
- **2026-05-18** First-principles + agentforce + post-retro alignment rooms with Codex (`t_89b3b45d-08d`, `t_7fec001e-890`, `t_a5cad7ea-da6`). ClawRoom positioned as the receipt/commitment layer in a larger agent stack (NOT the whole substrate); agentforce framing adopted as north star, not as a planning anchor. Out-of-scope made explicit in README (discovery / identity / reputation / payment / routing / execution / marketplace / SLA). Office-hours wedge interrogation surfaced the real demand: two named non-technical users asking for agent-to-agent pre-meeting sync. Added Lesson BZ (the wedge found the builder).
- **2026-06-13** Dress rehearsal + the E2E harness saga. A hand-driven cross-runtime rehearsal (cold `codex exec` gpt-5.5 host × opus guest) reached mutual close with both briefs and held boundaries — but the guest was an Agent subagent sharing the builder's auth (not a real cold CLI), surfacing that hand-driven runs contaminate the evidence. Built `evals/e2e/` (scrub + orchestrator + score + IQ-50 scenarios) to make the four clean-room guarantees mechanical. **Two external cross-platform audit rounds** then caught real defects the builder's own review missed: ambient MCP/AGENTS contamination, content-blind scoring (`closed=1` ≠ brief truth), work dirs inside the repo, a scorer reading transient `~/.clawroom-v4` state that the next run scrubbed (making a cited "PASS" unreproducible), `run-e2e.sh` exiting 0 on a failing score, no per-turn timeout, escalation checked against relay `owner_questions` which the state-only CLI `ask-owner` never writes. Fixes shipped across two rounds; the harness is converging to "honest smoke" but is not yet citable as production evidence. Added Lessons BX (a builder cannot validate its own test environment — the maintainer-truth substitution recurred three times in one session, each caught only by an outside checker) and BY (a test result is evidence only if self-contained and measuring the claim, not the mechanism). The irreducible validation remains the two-human / two-device alpha, which no single-machine harness replaces.
